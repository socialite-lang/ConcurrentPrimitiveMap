 ...
 * but the class is optimized for primitive float key type.
 *
 * Some of the methods in ConcurrentSkipListMap are not supported.
 * ConcurrentFloatSkipListMap supports the following APIs:
 *
 * - Map API methods
 ...
 *    public MapEntry.FloatKeyEntry<V> lowerEntry(float key)
 *    public float lowerKey(float key)
 *    public MapEntry.FloatKeyEntry<V> floorEntry(float key)
 *    public float floorKey(float key) {
 *    public MapEntry.FloatKeyEntry<V> ceilingEntry(float key)
 *    public float ceilingKey(float key) {
 *    public MapEntry.FloatKeyEntry<V> higherEntry(float key)
...
public class ConcurrentFloatSkipListMap<V> {
    ...
    private static final
        AtomicReferenceFieldUpdater<ConcurrentFloatSkipListMap, HeadIndex>
            headUpdater = AtomicReferenceFieldUpdater.newUpdater
            (ConcurrentFloatSkipListMap.class, HeadIndex.class, "head");
    ...
    static final class Node<V> {
         ...
         */
        Node(float key, Object value, Node<V> next) {
            this.key = key;
            this.value = value;
            this.next = next;
        }
        ...
        MapEntry.FloatKeyEntry<V> createSnapshot() {
            V v = getValidValue();
            if (v == null) return null;
            return new MapEntry.FloatKeyEntry<V>(key, v);
        }
    }
    ...
    boolean inHalfOpenRange(float key, float least, float fence) {
        return (compare(key, least) >= 0 && compare(key, fence) <  0);
    }

    boolean inOpenRange(float key, float least, float fence) {
        return (compare(key, least) >= 0 && compare(key, fence) <= 0);
    }

    /* ---------------- Traversal -------------- */

    public V findGrandPredecessor(float key, CallbackOnFound callback) {
    ...
    public V findGrandPredecessor(float key) {
        V v = findGrandPredecessorReally(key, nullCallback);
        if (v == BASE_HEADER)
            return null;
        return v;
    }
    V findGrandPredecessorReally(float key, CallbackOnFound callback) {
        for (;;) {
    ...
    Node<V> findGrandPredecessorWithIndex(float key) {
        for (;;) {
            Index<V> q = head;
            Index<V> r = q.right;
            for (;;) {
    ...
    private Node<V> findPredecessor(float key) {
        for (;;) {
            Index<V> q = head;
            Index<V> r = q.right;
            for (;;) {
    ...
    private Node<V> findNode(float key) {
        for (;;) {
    ...
    private V getUsingFindNode(float key) {
        for (;;) {
    ...
    private V doPut(float kkey, V value, boolean onlyIfAbsent) {
        float key = kkey;
        for (;;) {
            Node<V> b = findPredecessor(key);
            Node<V> n = b.next;
    ...
    final V doRemove(float okey, Object value) {
        float key = okey;
        for (;;) {
            Node<V> b = findPredecessor(key);
            Node<V> n = b.next;
    ...
    MapEntry.FloatKeyEntry<V> doRemoveFirstEntry() {
        for (;;) {
            Node<V> b = head.node;
            Node<V> n = b.next;
            if (n == null)
            ...
            if (!n.appendMarker(f) || !b.casNext(n, f))
                findFirst(); // retry
            clearIndexToFirst();
            return new MapEntry.FloatKeyEntry(n.key, (V)v);
        }
    }
    ...
    MapEntry.FloatKeyEntry<V> doRemoveLastEntry() {
        for (;;) {
            ...
            for (;;) {
                ...
                return new MapEntry.FloatKeyEntry<V>(key, (V)v);
            }
        }
    }
    ...
    Node<V> findNear(float kkey, int rel) {
        float key = kkey;
        for (;;) {
            Node<V> b = findPredecessor(key);
            Node<V> n = b.next;
    ...
    MapEntry.FloatKeyEntry<V> getNear(float key, int rel) {
        for (;;) {
            Node<V> n = findNear(key, rel);
            if (n == null)
                return null;
            MapEntry.FloatKeyEntry<V> e = n.createSnapshot();
            if (e != null)
                return e;
        }
    }
    ...
    public ConcurrentFloatSkipListMap() {
        initialize();
    }
