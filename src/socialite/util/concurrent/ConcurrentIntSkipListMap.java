 ...
 * but the class is optimized for primitive int key type.
 *
 * Some of the methods in ConcurrentSkipListMap are not supported.
 * ConcurrentIntSkipListMap supports the following APIs:
 *
 * - Map API methods
...
public class ConcurrentIntSkipListMap<V> {
    ...
    private static final
        AtomicReferenceFieldUpdater<ConcurrentIntSkipListMap, HeadIndex>
            headUpdater = AtomicReferenceFieldUpdater.newUpdater
            (ConcurrentIntSkipListMap.class, HeadIndex.class, "head");
    ...
    static final class Node<V> {
        ...
        MapEntry.IntKeyEntry<V> createSnapshot() {
            V v = getValidValue();
            if (v == null) return null;
            return new MapEntry.IntKeyEntry<V>(key, v);
        }
    }
    ...
    public V findGrandPredecessor(int key, CallbackOnFound callback) {
    ...
    public V findGrandPredecessor(int key) {
        V v = findGrandPredecessorReally(key, nullCallback);
        if (v == BASE_HEADER)
            return null;
        return v;
    }
    V findGrandPredecessorReally(int key, CallbackOnFound callback) {
        for (;;) {
    ...
    Node<V> findGrandPredecessorWithIndex(int key) {
        for (;;) {
            Index<V> q = head;
            Index<V> r = q.right;
            for (;;) {
    ...
    MapEntry.IntKeyEntry<V> doRemoveFirstEntry() {
        for (;;) {
            Node<V> b = head.node;
            Node<V> n = b.next;
            if (n == null)
            ...
            if (!n.appendMarker(f) || !b.casNext(n, f))
                findFirst(); // retry
            clearIndexToFirst();
            return new MapEntry.IntKeyEntry(n.key, (V)v);
        }
    }
    ...
    MapEntry.IntKeyEntry<V> doRemoveLastEntry() {
        for (;;) {
            ...
            for (;;) {
                ...
                return new MapEntry.IntKeyEntry<V>(key, (V)v);
            }
        }
    }
    ...
    MapEntry.IntKeyEntry<V> getNear(int key, int rel) {
        for (;;) {
            Node<V> n = findNear(key, rel);
            if (n == null)
                return null;
            MapEntry.IntKeyEntry<V> e = n.createSnapshot();
            if (e != null)
                return e;
        }
    }
    ...
    public ConcurrentIntSkipListMap() {
        initialize();
    }
